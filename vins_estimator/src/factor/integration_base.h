...
class IntegrationORB
{
    public:
    IntegrationORB() = delete;
    IntegrationORB(const Eigen::Vector3d &_acc_0, const Eigen::Vector3d &_gyr_0,
                    const Eigen::Vector3d &_linearized_ba, const Eigen::Vector3d &_linearized_bg)
        : acc_0{_acc_0}, gyr_0{_gyr_0}, linearized_acc{_acc_0}, linearized_gyr{_gyr_0},
          linearized_ba{_linearized_ba}, linearized_bg{_linearized_bg},
            jacobian{Eigen::Matrix<double, 15, 15>::Identity()}, covariance{Eigen::Matrix<double, 15, 15>::Zero()},
...

    {
        ...
        Nga.block<3,3>(0,0) *= GYR_N;
        Nga.block<3,3>(3,3) *= ACC_N;

        NgaWalk.block<3,3>(0,0) *= GYR_W;
        NgaWalk.block<3,3>(3,3) *= ACC_W;
    ...
    Eigen::Matrix3d JrInverse(const Eigen::Vector3d &vec)
    {
        double x = vec[0];
        double y = vec[1];
        double z = vec[2];
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        if(update_jacobian)
        {
            Eigen::Vector3d local_acc =  ((_acc_0 - linearized_ba) + (_acc_1 - linearized_ba))/2.0;
            Eigen::Quaterniond q_(1, un_gyr(0) * _dt / 2, un_gyr(1) * _dt / 2, un_gyr(2) * _dt / 2);
            Eigen::Matrix3d un_acc_skew;
            un_acc_skew << 0, -local_acc[2],  local_acc[1] , local_acc[2], 0, -local_acc[0], -local_acc[1], local_acc[0], 0;
            Eigen::Matrix3d jr = Jr(Eigen::Vector3d(un_gyr(0) * _dt, un_gyr(1) * _dt, un_gyr(2) * _dt));
            ...
            A.block<3,3>(0,0) = 0.5*delta_q.toRotationMatrix()*un_acc_skew*_dt*_dt;
            A.block<3,3>(0,3) = Eigen::Matrix3d::Identity()*_dt;
            A.block<3,3>(0,6) = Eigen::Matrix3d::Identity();
            A.block<3,3>(3,0) = delta_q.toRotationMatrix()*un_acc_skew*_dt;
            A.block<3,3>(3,3) = Eigen::Matrix3d::Identity();
            A.block<3,3>(6,0) = (q_.toRotationMatrix()).transpose();
            
            B.block<3,3>(0,0) = jr*_dt;
            B.block<3,3>(0,3) = delta_q.toRotationMatrix()*_dt;
            B.block<3,3>(6,3) = 0.5*delta_q.toRotationMatrix()*_dt*_dt;

            covariance.block<9,9>(0,0) = A* covariance.block<9,9>(0,0)*A.transpose() + B*Nga*B.transpose();
            covariance.block<6,6>(9,9) = covariance.block<6,6>(9,9) + NgaWalk;

            JPa = JPa + JVa*dt -0.5f*delta_q.toRotationMatrix()*_dt*_dt;
            JPg = JPg + JVg*dt -0.5f*delta_q.toRotationMatrix()*_dt*_dt*un_acc_skew*JRg;
            JVa = JVa - delta_q.toRotationMatrix()*_dt;
            JVg = JVg - delta_q.toRotationMatrix()*_dt*un_acc_skew*JRg;
            JRg =(q_.toRotationMatrix()).transpose()*JRg - jr*_dt;
                                          ...
                                          const Eigen::Vector3d &Pj, const Eigen::Quaterniond &Qj, const Eigen::Vector3d &Vj, const Eigen::Vector3d &Baj, const Eigen::Vector3d &Bgj)
    {
        ...
        Eigen::Matrix3d dp_dba = JPa;
        Eigen::Matrix3d dp_dbg = JPg;

        Eigen::Matrix3d dq_dbg = JRg;

        Eigen::Matrix3d dv_dba = JVa;
        Eigen::Matrix3d dv_dbg = JVg;
    ...
    Eigen::Matrix3d JPa, JPg, JVa, JVg, JRg;
};
